...
class UVTestingStrategy(AbstractStrategy):
    """
    ...
    """
    def __init__(self, cfg: ConfigImageProcessor):
        super().__init__(cfg=cfg)
        self.path_to_save = Path("/media/hslab/Data/ImageData/Idris/2024-04-27")
        ...
        self.proj_img[self.proj_img.shape[0] // 4:self.proj_img.shape[0] * 3 // 4, :] = 255
        self.proj_times = {i: val for i, val in enumerate([0, 1, 1, 5, 10, 30, 60])}  # in seconds
    ...
    def _initialise(self) -> List[AutomatonCommand]:
        ...
        logger.info(
            f"Projecting UV on {len(self.proj_times)} different positions with exposure times {self.proj_times}."
        )
