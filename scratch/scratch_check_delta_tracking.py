...
def get_label_color(label, cmap):
    # Get the color from the cmap based on the label value
    color = cmap(label)
    # Convert the RGBA color to a tuple of RGB values
...
axs[0].imshow(pos1_exp.rois[i_roi].label_stack[i_period-1], cmap='gray', vmin=0, vmax=1)
axs[0].set_title(f"POS={i_pos}, ROI={i_roi}, t={i_period-1}")
label_img_prev = pos1_exp.rois[i_roi].label_stack[i_period-1]
for i in range(label_img_prev.shape[0]):
    for j in range(label_img_prev.shape[1]):
        label = label_img_prev[i, j]
        if label > 0 and i % 5 == 0 and j % 5 == 0:
            label_color = get_label_color(label, cmap)
            axs[0].text(j, i, str(label), color=label_color, ha='center', va='center')

axs[1].imshow(pos1_exp.rois[i_roi].label_stack[i_period], cmap='gray', vmin=0, vmax=1)
axs[1].set_title(f"POS={i_pos}, ROI={i_roi}, t={i_period}")
label_img_curr = pos1_exp.rois[i_roi].label_stack[i_period]
for i in range(label_img_curr.shape[0]):
    for j in range(label_img_curr.shape[1]):
        label = label_img_curr[i, j]
        if label > 0 and i % 5 == 0 and j % 5 == 0:
            label_color = get_label_color(label, cmap)
            axs[1].text(j, i, str(label), color=label_color, ha='center', va='center')

exp_cell_ids_t = [_id for (_id, x) in pos1_exp.rois[i_roi].lineage.cells.items() if x.first_frame <= i_period+1 <= x.last_frame]
exp_cell_ids_tm1 = [_id for (_id, x) in pos1_exp.rois[i_roi].lineage.cells.items() if x.first_frame <= i_period <= x.last_frame]



axs[1, 0].imshow(pos1_exp.rois[i_roi].img_stack[i_period-1], cmap='gray', vmin=0, vmax=1)
axs[1, 0].set_title(f"POS={i_pos}, ROI={i_roi}, t={i_period-1}")
axs[1, 1].imshow(pos1_exp.rois[i_roi].img_stack[i_period], cmap='gray', vmin=0, vmax=1)
axs[1, 1].set_title(f"POS={i_pos}, ROI={i_roi}, t={i_period}")
axs[2, 0].imshow(pos1_exp.rois[i_roi].seg_stack[i_period-1], cmap='gray', vmin=0, vmax=1)
axs[2, 0].set_title(f"POS={i_pos}, ROI={i_roi}, t={i_period-1}")
axs[2, 1].imshow(pos1_exp.rois[i_roi].seg_stack[i_period], cmap='gray', vmin=0, vmax=1)
