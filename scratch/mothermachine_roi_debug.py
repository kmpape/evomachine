...
if use_delta_cam == False:
    ...
    IMAGE_CONFIG_DEFAULT.pxl_horiz = 3200
    IMAGE_CONFIG_DEFAULT.pxl_dtype = np.dtype("uint16")
else:
    this_cam = DeltaCamera(cfg_device=DEVICE_CONFIG_DELTA_SIM)
    IMAGE_CONFIG_DEFAULT.pxl_vert = 520
    IMAGE_CONFIG_DEFAULT.pxl_horiz = 696
    IMAGE_CONFIG_DEFAULT.pxl_dtype = np.dtype("float32")

automaton = Automaton(
    cfg_device=DEVICE_CONFIG_EVO_TEST if use_delta_cam is False else DEVICE_CONFIG_DELTA_SIM,
    cfg_image=IMAGE_CONFIG_DEFAULT,
...
for i_pos in range(automaton._cfg_device.num_pos):
    automaton._camera.move_to_pos(i_pos=i_pos)
    for i_chan in range(automaton._cfg_device.num_chan):
        tmp = automaton._camera.get_frame(i_chan=i_chan, i_period=0)
        automaton._ref_frames[i_pos][i_chan, :, :] = tmp


i_pos = 0
pos = automaton._pos_processor[i_pos]
pos.initialise(automaton._ref_frames[i_pos])

fig, axs = plt.subplots(1, 2)
axs[0].imshow(pos.tmp_reference_orig)
axs[0].set_title('Reference')
axs[1].imshow(pos.tmp_rois_mask)
axs[1].set_title('rois_mask')

#  inputs, win_y, win_x = delta.utils.create_windows(image=pos.tmp_reference_orig, target_size=pos.config.target_size_rois)
