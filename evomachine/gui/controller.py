...
class EvoMachineGuiController(QObject):
    ...
    def refresh_acquisition_files(self, directory: str | None = None) -> None:
        payload = {} if directory is None else {"directory": directory}
        self._send(GuiCommandType.ACQUISITION_LIST_FILES, payload)

    def set_acquisition_directory(self, directory: str) -> None:
        self._send(GuiCommandType.ACQUISITION_SET_DIRECTORY, {"directory": directory})
    ...
    def _handle_response(self, response: GuiResponse) -> None:
        ...
        if "loading_directory" in payload:
            self.acquisition_loading_directory_received.emit(payload["loading_directory"])
        if "frame" in payload:
