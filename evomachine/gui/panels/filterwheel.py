...
from PyQt5.QtWidgets import QComboBox, QGridLayout, QGroupBox, QLabel, QPushButton, QVBoxLayout, QWidget
...
class FilterWheelPanel(QGroupBox):
    ...
    def _set_filter_wheel(self) -> None:
        ...
        if not filter_name:
            ...
            return
        self.status_label.setText(f"Setting filter wheel to {self._format_filter_name(filter_name)}.")
        self.controller.set_filter_wheel(filter_wheel=filter_name)

    def update_status(self, payload: dict) -> None:
        self._set_available_filters(payload.get("available_filters", []))
    ...
    def _show_error(self, error: str) -> None:
        if "filter wheel" in error.lower() or self.status_label.text().startswith("Setting filter wheel"):
